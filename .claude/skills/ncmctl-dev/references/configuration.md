...
- The persistent jar is not installed as `Client.GetClient().Jar`; it is owned by the API Cookie transport, which reads it for each actual request URL and immediately writes each response's `Set-Cookie` back. Use `Client.GetCookies` and `Client.SetCookies` for jar access.
- `Client.Close` closes the Cookie transport to new `RoundTrip` calls and starts one shared shutdown that waits in-flight Cookie transport calls, closes idle connections, and then performs the final jar export. A canceled caller returns early while that shutdown continues, and a later `Close` waits for the same result. It does not wait for the complete lifetime of `Request`, `Upload`, or `Download`; callers must finish those before closing the client.
...
The root command creates `pkg/log.Logger` after config and CLI overrides. Close it at the command boundary; remember that Cobra skips `PersistentPostRunE` when `RunE` fails, so resources created inside a command need their own cleanup path. Package-level logging uses `log.GetDefault()` and `log.SetDefault()`; code that accessed the removed `log.Default` variable must migrate to those accessors.
