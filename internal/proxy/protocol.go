...
func decryptEAPIResponse(body []byte, maxBodyBytes int64) (plaintext []byte, gzipDecoded bool, encoding string, err error) {
...
func gunzipLimited(data []byte, limit int64) ([]byte, error) {
	...
	decoded, exceeded, err := readLimited(reader, limit)
	if err != nil {
		return nil, err
	}
	if exceeded {
