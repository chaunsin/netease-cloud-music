...
func loadOrCreateCA(certPath, keyPath string, requirePrivateCAPath bool) (*tls.Certificate, bool, error) {
	if certPath == "" || keyPath == "" {
		return nil, false, errors.New("CA certificate and key paths are required")
	}

	if filepath.Clean(certPath) == filepath.Clean(keyPath) {
		return nil, false, errors.New("CA certificate and key paths must be different")
	}
	...
	if certExists != keyExists {
		return nil, false, errors.New("CA certificate and private key must either both exist or both be absent")
	}
	...
	if certExists {
		...
		if secureErr := secureCAPrivateKey(keyPath); secureErr != nil {
			return nil, false, fmt.Errorf("secure CA private key: %w", secureErr)
		}

		ca, loadErr := loadCA(certPath, keyPath)
		if loadErr != nil {
...
func loadCA(certPath, keyPath string) (*tls.Certificate, error) {
	...
	if len(ca.Certificate) == 0 {
		return nil, errors.New("CA certificate chain is empty")
	}
...
func validateCA(ca *tls.Certificate, now time.Time) error {
	if ca == nil || ca.Leaf == nil {
		return errors.New("CA leaf certificate is missing")
	}
	...
	if !ok {
		return errors.New("CA private key does not implement crypto.Signer")
	}
