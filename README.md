...
本项目仅供个人学习使用,切勿用于商业用途、非法用途使用！！！如有侵权,即删。
...
由于本库依赖三方库进行设置音乐tag信息，存在内存占用过高的问题，目前正在寻找解决方案。详情: https://github.com/go-flac/go-flac/issues/5

虽然目前暂时没有解决方案,但是有个折中方案,可以在解析时指定`--tag`(默认是开启)命令参数选项,用于关闭音乐tag设置即可解决内存占用过高问题.
但这有个问题,解析出来得音乐中不会携带音乐tag信息,比如音乐标题、封面图、专辑等需要衡量。
